...
{
    var $_selectedHost;
    var $_hosts; ///< searchd hosts (default is "['localhost']")
    var $_loads; ///< searchd loads (default is "[100]")
    ...
    function SphinxClient()
    {
        ...
        $this->_selectedHost = 'localhost';
        $this->_hosts        = array('localhost');
        $this->_loads        = array('100');
        $this->_port         = 9312;
        $this->_path         = false;
        $this->_socket       = false;
     ...
     * @param int   $port
     */
    function SetServer(array $hosts, array $loads = array(), $port = 0)
    {
        assert(is_array($hosts));
        assert(is_array($loads));
        assert(is_int($port));

        $this->_selectedHost = $this->selectSphinxHost($hosts, $loads, $port);
        $this->_port = $port;
        $this->_path = '';
    }
     ...
     * @throws Exception
     */
    private function selectSphinxHost(array $hosts, array $loads, $port)
    {
        $total_percentage = 0;
        $tries            = 1;
        $connections      = array();
        ...
        while ($tries < 10) {
            ...
            for ($i = 0, $n = count($hosts); $i < $n; $i++) {
                $ip_address = $hosts[$i];
                $connection = ($fp = @fsockopen($ip_address, $port, $errno, $errstr, floatval(0.2)));

                if ($connection !== false) {
                    $index = count($connections);

                    $connections[$index]['ip_address'] = $ip_address;
                    $connections[$index]['percentage'] = isset($loads[$i]) ? $loads[$i] : 0;

                    $total_percentage += $connections[$index]['percentage'];
                }
            }
        ...
        if (count($connections) == 0) {
            throw new Exception('Unable to connect to any sphinx server - last error: ');
        }

        //Define value between 0 and 100
        $random_value         = rand(0, 100);
        $new_total_percentage = 0;

        //Define real percentages based on number of valid connections; if percentages are 30% and 70% and connection 2 is invalid, connection 1 will get 100%
        for ($i = 0; $i < count($connections); $i++) {
            $connections[$i]['percentage']  = round((100 / $total_percentage) * $connections[$i]['percentage'], 0) + $new_total_percentage;
            $new_total_percentage           = $connections[$i]['percentage'];
        }

        $serverIdx = 0;

        //Check if random value is in percentage range
        for ($i = 0; $i < count($connections); $i++) {
            if ($random_value <= $connections[$i]['percentage']) {
                $serverIdx = $i;
                break;
            }
        }

        return $connections[$serverIdx]['ip_address'];
    }
    ...
    function _Connect()
    {
        ...
        if (!$fp) {
            if ($this->_path) {
                ...
                $location = "{$this->_selectedHost}:{$this->_port}";
            }
